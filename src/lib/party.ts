...
import { utils, type HostsMap } from './utils.js';
 ...
 */
const setup = (setupOptions?: PartyOptions): PartyAPI => {
 ...
 */
const add = async (ip: string, hostNames: string | string[]): Promise<void> => {
  ip = (ip || '').trim();
 ...
 */
const remove = async (ips: string | string[]): Promise<void> => {
 ...
 */
const purge = async (hostNames: string | string[]): Promise<void> => {
  ...
  for (const hostName of hostNamesArray) {
    ...
    for (const [ip, hostList] of Object.entries(hosts)) {
      // purge the matching
      hosts[ip] = hostList.filter(host => 
        host.toLowerCase().trim() !== hostName.toLowerCase().trim()
      );
 ...
 */
const list = async (filter?: string): Promise<HostsMap> => {
 ...
 */
const getHostFilePath = async (): Promise<string> => {
  let filePath: string;
 ...
 */
const loadHosts = async (filter?: string): Promise<HostsMap> => {
  ...
  file.toString().split("\n").forEach((line: string) => {
    ...
    if (filter) {
      const matching = ip.filter((host: string) => host.indexOf(filter) > -1);
      if (matching.length) {
        ip = matching;
      } else {
        return;
 ...
 */
const saveToFile = async (hosts: HostsMap): Promise<void> => {
